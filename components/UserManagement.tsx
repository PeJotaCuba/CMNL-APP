...
import { ArrowLeft, Upload, Trash2, UserPlus, Search, FileText, Info, Edit2 } from 'lucide-react';
...
const UserManagement: React.FC<Props> = ({ onBack, users, setUsers, setHistoryContent, setAboutContent }) => {
  ...
  const startEditUser = (user: User) => {
    setNewUser(user);
    setIsEditing(true);
    setActiveTab('users'); // Ensure we are on the tab
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'users' | 'history' | 'about') => {
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="w-full md:w-1/3 bg-[#2C1B15] p-6 overflow-y-auto border-r border-[#9E7649]/10">
                    ...
                    <form onSubmit={addUser} className="flex flex-col gap-3 mb-8">
                        ...
                        <input name="username" placeholder="Nombre de usuario" value={newUser.username} onChange={handleInputChange} disabled={isEditing} className={`bg-[#1A100C] border border-[#9E7649]/20 rounded-lg p-3 text-sm focus:border-[#9E7649] outline-none ${isEditing ? 'opacity-50' : ''}`} required />
                        <input name="mobile" placeholder="Número de móvil" value={newUser.mobile} onChange={handleInputChange} className="bg-[#1A100C] border border-[#9E7649]/20 rounded-lg p-3 text-sm focus:border-[#9E7649] outline-none" />
                        ...
                        <div className="flex gap-2">
                            ...
                            {isEditing && (
                                <button type="button" onClick={() => { setIsEditing(false); setNewUser({ name: '', username: '', mobile: '', password: '', role: 'worker' }); }} className="bg-white/10 text-white px-4 rounded-lg mt-2 font-bold">
                                    Cancelar
                                </button>
                            )}
                        </div>
