...
const UserManagement: React.FC<Props> = ({ 
  ...
  return (
    <div className="flex flex-col h-screen bg-[#1A100C] text-[#E8DCCF] font-display">
      ...
      {/* Tabs */}
      <div className="flex border-b border-[#9E7649]/20">
          <button 
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {activeTab === 'users' && (
            // Changed layout: reverse col on mobile to put List on top, Form on bottom. 
            // On desktop (md), row layout (Form left, List right - but due to reverse in markup order, we need to be careful)
            // HTML Order: List, Form. 
            // Mobile (col): List (top), Form (bottom).
            // Desktop (row): List (left), Form (right) - wait, standard dashboard usually has List prominent?
            // Let's stick to standard dashboard: List is the main content. Form is side panel.
            // On Desktop: List Left, Form Right (w-1/3).
            // On Mobile: List Top, Form Bottom.
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                
                {/* List - Comes First in HTML for Mobile Top placement */}
                <div className="flex-1 bg-[#1A100C] flex flex-col overflow-hidden order-1 md:order-2">
                ...
                {/* Form - Comes Second in HTML for Mobile Bottom placement */}
                <div className="w-full md:w-1/3 bg-[#2C1B15] p-6 overflow-y-auto border-t md:border-t-0 md:border-r border-[#9E7649]/10 order-2 md:order-1">
                    ...
                    <form onSubmit={addUser} className="flex flex-col gap-3 mb-8">
                        ...
                        <div className="flex gap-2">
                             <input name="username" placeholder="Nombre de usuario" value={newUser.username} onChange={handleInputChange} disabled={isEditing} className={`flex-1 bg-[#1A100C] border border-[#9E7649]/20 rounded-lg p-3 text-sm focus:border-[#9E7649] outline-none ${isEditing ? 'opacity-50' : ''}`} required />
                             <input name="mobile" placeholder="Móvil (5xxxxxxx)" value={newUser.mobile} onChange={handleInputChange} className="flex-1 bg-[#1A100C] border border-[#9E7649]/20 rounded-lg p-3 text-sm focus:border-[#9E7649] outline-none" />
                        </div>
        ...
        {activeTab === 'content' && (
            <div className="flex-1 overflow-y-auto p-6 bg-[#1A100C]">
